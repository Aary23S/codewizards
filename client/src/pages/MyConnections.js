...
const MyConnections = () => {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      try {
        const res = isStudent ? await getMyMentors() : await getMyMentees();
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="mb-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/60">
            {isStudent
              ? "View your active mentors and access their shared contact channels to connect externally."
              : "Review your active mentees, view their profile summaries, coding stats, and shared contact details."}
          </p>
        </div>
        ...
        {connections.length === 0 ? (
          <div className={`${shellCard} p-8 text-center text-white/55`}>
            ...
            <p className="mt-2 text-sm text-white/40">
              {isStudent
                ? "Send a mentorship request to a mentor from the directory to get started."
                : "Active connections will appear here once you accept pending student requests."}
            </p>
            {isStudent && (
              <Link
                to="/connect"
          ...
          <div className="grid gap-6">
            {connections.map((conn) => {
              // The connected user model is either studentId or mentorId
              const targetUser = isStudent ? conn.mentorId : conn.studentId;
                  ...
                  <div className="p-6 md:p-7">
                    ...
                    {isExpanded && (
                      <div className="mt-6 border-t border-white/10 pt-6">
                        ...
                        {!isStudent && targetUser.externalStats && (
                        ...
                        <div>
                          <p className="text-[10px] uppercase tracking-[0.25em] text-white/45 mb-3">Allowed Contact Info</p>
                          {isLoadingC ? (
                            <p className="text-sm text-white/40">Loading contact information...</p>
                          ) : Object.keys(connContacts).length === 0 ? (
                            <p className="text-sm text-white/40">
                              No contact details were shared by this user. You can try reaching them via their LinkedIn/GitHub links.
                            </p>
                          ) : (
