        ...
        <div className="mt-4 border-t border-gray-800 pt-4 flex flex-col gap-3">
                ...
                <div key={r._id} className="flex gap-3">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`text-xs px-1.5 py-0.5 rounded capitalize ${r.author?.role === "senior" || r.author?.role === "alumni"
            ...
            {user && doubt.author?._id === user._id && (
                <button onClick={() => onResolve(doubt._id)}
                    className={`text-xs w-fit px-3 py-1 rounded-lg border transition-colors ${doubt.resolved
...
const DoubtCard = ({ doubt, user, onReply, onResolve, onUpvote }) => {
    ...
    return (
        <div className={`border rounded-xl bg-gray-900 transition-colors ${doubt.resolved ? "border-green-900" : "border-gray-800 hover:border-gray-600"
            }`}>
            <div className="p-6">
                {/* Header */}
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {["all", "open", "resolved"].map((f) => (
                    <button key={f} onClick={() => setResolvedFilter(f)}
                        className={`text-xs px-4 py-2 rounded-full border capitalize transition-colors ${resolvedFilter === f ? "bg-white text-black border-white font-semibold" : "border-gray-700 text-gray-400 hover:border-gray-500"
                            }`}>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
                {DOMAINS.map((d) => (
                    <button key={d} onClick={() => setDomain(d)}
                        className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${domain === d ? "bg-white text-black border-white font-semibold" : "border-gray-700 text-gray-400 hover:border-gray-500"
                            }`}>
                        {d}
                    </button>
                ))}
