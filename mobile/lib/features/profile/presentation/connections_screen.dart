...
class _ConnectionsScreenState extends State<ConnectionsScreen> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (loadingGoal)
                    ...
                    ...connGoals.map((g) => _buildGoalItem(connId, g, !isMentorCard)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildGoalItem(String connId, Map<String, dynamic> goal, bool isMentor) {
    ...
    final tasksList = List<Map<String, dynamic>>.from(
      (goal['tasks'] as List? ?? const []).map((e) => Map<String, dynamic>.from(e as Map)),
    );

    final completedCount = tasksList.where((t) => t['isCompleted'] == true).length;
      ...
      child: Column(
        ...
        children: [
          ...
          if (description != null && description.trim().isNotEmpty) ...[
            const SizedBox(height: 4),
            Text(description, style: TextStyle(fontSize: 12, color: Colors.white.withAlpha(120))),
          ],
          const SizedBox(height: 12),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
          ...
          if (tasksList.isNotEmpty) ...[
            ...
            ...tasksList.map((task) {
              ...
              return InkWell(
                onTap: () => _toggleTask(connId, goalId, taskId, isCompleted, tasksList),
                child: Padding(
  ...
  Future<void> _toggleTask(
    ...
    try {
      await context.read<ProfileRepository>().updateGoal(goalId, {'tasks': updatedTasks});
    } catch (e) {
      // Revert on error
  ...
  Future<void> _deleteGoal(String connId, String goalId) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this mentorship goal?'),
        actions: [
    ...
    try {
      ...
      setState(() {
        _goals[connId] = _goals[connId]!.where((g) => g['_id'] != goalId).toList();
      });
    } catch (e) {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text('Tasks', style: TextStyle(fontSize: 12, color: Colors.white38)),
                const SizedBox(height: 8),
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                try {
                  final newGoal = await context.read<ProfileRepository>().createGoal(connId, {
                    'title': title,
                  ...
                  if (ctx.mounted) {
                    ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Failed to create goal.')));
                  }
                }
              },
  ...
  Future<void> _loadGoalsFor(String connId) async {
    try {
      final goalsData = await context.read<ProfileRepository>().fetchGoals(connId);
      if (mounted) {
        setState(() {
          _goals[connId] = goalsData;
        });
